...
for data_type in ('cases', 'deaths'):
    ...
    for country, color in (('Italy', 'r'), ('US', 'b'), ('China', 'g'), ('Iran', 'y'), ('Spain', 'purple'), ('India', 'black'), ('Brazil', 'pink')):
        def filter(x):
        ...
        plt.ylim([0, .5])
    plt.legend(legends)
    plt.title('Logistic Model Predictions of %s for %s' %
              (data_type, today), fontsize=40)
